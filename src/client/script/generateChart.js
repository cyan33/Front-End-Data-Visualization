...
export function generateHeatMap() {
  function getVirtualData(year = '2017') {
    const date = +echarts.number.parseDate(`${year}-01-01`);
    const end = +echarts.number.parseDate(`${+year + 1}-01-01`);
    const dayTime = 3600 * 24 * 1000; // seconds * hour * millseconds
    const data = [];
    for (let time = date; time < end; time += dayTime) {
      data.push([
        echarts.format.formatTime('yyyy-MM-dd', time),
    ...
    return data; // [['2017-01-01', 4603], ['2017-01-02', 6785], ...],length === 365
  }

  const data = getVirtualData('2017')
  ...
  myChart.setOption({
    ...
    series: [
      ...
      },
      {
        ...
        coordinateSystem: 'calendar',
        calendarIndex: 1,
        data: data.sort((a, b) => b[1] - a[1]).slice(0, 12),
        symbolSize(val) {
          return val[1] / 500;
        },
        showEffectOn: 'render',
