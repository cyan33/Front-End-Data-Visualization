...
function parseWeatherData(html) {
  ...
  dataBlocks.each((i, item) => {
    ...
    if (!~Object.keys(geoCoordMap).indexOf(name)) return
    json.push({
      name, // city name
      value: geoCoordMap[name].concat($(item).find('td:nth-child(6)').text()), // ['三亚', 109.511909, 18.252847, 5]
    })
  })
  return json
}
...
function parseAPI(json) {
  return JSON.parse(json).filter(item => !!geoCoordMap[item.area])
    .map(item => ({
      name: item.area,
      value: geoCoordMap[item.area].concat(item.pm2_5),
    }))
}
...
function getHardCodedData() {
  ...
  // console.log(JSON.stringify({ data, geoCoordMap }))
  return { data, geoCoordMap }
}
